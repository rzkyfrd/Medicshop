            ...
            <div class="py-2 px-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <section>
                    <div class="mx-auto max-w-screen-xl grid grid-cols-3 gap-4 px-4 py-4 sm:px-6 sm:py-12 lg:px-8">
                        <div class="border px-4 py-1 rounded max-w-3xl col-span-2 overflow-auto box-content {{$cart->count() == 0 ? 'flex items-center justify-center' : ''}}"
                            ...
                            <ul class="space-y-4 mt-3">
                                @forelse ($cart as $key => $row)
                                        ...
                                        <div class="flex flex-1 items-center justify-end gap-2">
                                            <form method="POST" action="{{ route('cart.update', $row) }}">
                                                ...
                                                <div>
                                                    ...
                                                    <div x-data="{ quantity: {{ $row->quantity }} }"
                                                        ...
                                                        <button x-on:click="quantity++" type="submit"
                                                            class="size-6 leading-6 text-gray-600 transition hover:opacity-75">
                                                            +
                                                        </button>
                                                    </div>
                                            ...
                                            <form method="POST" action="{{ route('cart.destroy', $row) }}">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    @endif
                                @empty
                                    <h2 class="text-center text-2xl font-bold"> Your Cart Is Empty</h2>
                                @endforelse
                            </ul>
                        </div>
