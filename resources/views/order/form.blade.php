    ...
    <div class="py-4">
        <div class="w-3/4 mx-auto sm:px-6 lg:px-8">
            <div class="px-10 py-4 mb-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <hr class="mt-2 mb-4">
                <form action="{{ $order->exists ? route('order.update', $order) : route('order.store') }}" method="POST"
                        ...
                        <div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" id="name"
                                    placeholder="Input your name receipt..."
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="address" id="address" placeholder="Input your address..."
                                    value="{{ old('address') }}"
                                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" id="contact" name="contact"
                                    placeholder="input your number contact..." value="{{ old('contact') }}"
                                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                            </div>
                        </div>
                        <div x-data="{ method: '{{ old('payment_method') }}', city: '{{ old('city') }}' }">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="city" id="city" x-model="city"
                                    ...
                                    <option {{ old('city') === 'Surabaya' ? 'selected' : '' }} value="Surabaya">Surabaya
                                    </option>
                                    <option {{ old('city') === 'Jakart' ? 'selected' : '' }} value="Jakarta">Jakarta
                                    </option>
                                    <option {{ old('city') === 'Bandung' ? 'selected' : '' }} value="Bandung">Bandung
                                    </option>
                                    <option {{ old('city') === 'Malang' ? 'selected' : '' }} value="Malang">Malang
                                    </option>
                                    <option {{ old('city') === 'Solo' ? 'selected' : '' }} value="Solo">Solo</option>
                                    <option {{ old('city') === 'Semarang' ? 'selected' : '' }} value="Semarang">
                                        Semarang</option>
                                    <option {{ old('city') === 'Jogja' ? 'selected' : '' }} value="Jogja">Jogja
                                    </option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="payment_method" class="block text-xs font-medium text-gray-700"> Payment
                                    Method </label>
                                <select name="payment_method" id="payment_method" x-model="method"
                                    class="mt-1 w-full rounded-lg border-gray-300 text-gray-700 sm:text-sm">
                                    <option>Please select payment method</option>
                                    <option {{ old('payment_method') === 'prepaid' ? 'selected' : '' }}
                                        value="prepaid">Prepaid</option>
                                    <option {{ old('payment_method') === 'postpaid' ? 'selected' : '' }}
                                        x-show="city === 'Surabaya'" value="postpaid">Postpaid</option>
                                </select>
                            </div>
                            <div class="mb-3" x-show="method === 'prepaid'">
                                ...
                                <select name="bank" id="bank"
                                    ...
                                    <option {{ old('bank') === 'credit' ? 'selected' : '' }} value="credit">Credit
                                    </option>
                                    <option {{ old('bank') === 'debit' ? 'selected' : '' }} value="debit">Debit
                                    </option>
                                    <option {{ old('bank') === 'paypal' ? 'selected' : '' }} value="paypal">Paypal
                                    </option>
                                </select>
                            </div>
                ...
                <div class="overflow-auto h-44">
                    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                        <thead class="ltr:text-left rtl:text-right">
                            <tr>
                                <th class="sticky top-0 bg-white whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                    Name</th>
                                <th
                                    class="sticky top-0 bg-white whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                    Category</th>
                                <th
                                    class="sticky top-0 bg-white whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                    Quantity</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($cart as $row)
                                <tr class="odd:bg-gray-50">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        @currency($row->product->price * $row->quantity)
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
